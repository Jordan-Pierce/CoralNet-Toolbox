...
from PyQt5.QtGui import QMouseEvent, QImage, QPixmap, QColor, QPen, QBrush, QPolygonF, QCursor
from PyQt5.QtWidgets import (QFileDialog, QApplication, QGraphicsView, QGraphicsScene, QMessageBox, QCheckBox,
                             ...
                             QDialogButtonBox, QButtonGroup, QGraphicsPolygonItem)
...
class AnnotationWindow(QGraphicsView):
    ...
    def __init__(self, main_window, parent=None):
        ...
        self.polygon_points = []  # List to store points for polygon annotation
            ...
            try:
                ...
                for image_path, annotations in filtered_annotations.items():
                    for annotation_data in annotations:
                        ...
                        if not all(key in annotation_data for key in keys):
                            continue
                        if 'center_xy' in annotation_data and 'annotation_size' in annotation_data:
                            annotation = PatchAnnotation.from_dict(annotation_data)
                        elif 'points' in annotation_data:
                            annotation = PolygonAnnotation.from_dict(annotation_data)
                        else:
                            continue
        ...
        if scene_pos:
            ...
            if not self.cursor_annotation:
                if self.selected_tool == "patch":
                    self.cursor_annotation = self.create_patch_annotation(scene_pos)
                    self.cursor_annotation.create_graphics_item(self.scene)
                elif self.selected_tool == "polygon":
                    if len(self.polygon_points) > 0:
                        temp_points = self.polygon_points + [scene_pos]
                        self.cursor_annotation = self.create_polygon_annotation(temp_points, finished=False)
                        self.cursor_annotation.create_graphics_item(self.scene)
            else:
                if self.selected_tool == "patch":
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if self.active_image:
            ...
            if event.button() == Qt.LeftButton and self.selected_tool == "select":
                ...
                # Collect both rect and polygon items
                ...
                # Combine and sort the items by zValue
                all_items = rect_items + polygon_items
                all_items.sort(key=lambda item: item.zValue(), reverse=True)

                for item in all_items:
                    annotation_id = item.data(0)  # Retrieve the UUID from the graphics item's data
            ...
            elif self.selected_tool == "polygon" and event.button() == Qt.LeftButton and self.check_label_selected():
                ...
                if len(self.polygon_points) > 1:
                    if self.cursor_annotation:
                        self.cursor_annotation.delete()
                    self.cursor_annotation = self.create_polygon_annotation(self.polygon_points, finished=False)
                    self.cursor_annotation.create_graphics_item(self.scene)
    ...
    def keyPressEvent(self, event):
        if self.selected_tool == "polygon" and event.key() == Qt.Key_Space:
    ...
    def create_polygon_annotation(self, points, finished=True):
        if finished:
