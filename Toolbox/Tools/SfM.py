...
def print_sfm_progress(p):
    ...
    print("progress: {}/{}".format(int(p), 100), flush=True)
...
def sfm_workflow(args):
    ...
    print("\n###############################################", flush=True)
    print("Structure from Motion", flush=True)
    print("###############################################\n", flush=True)
    ...
    if args.project_dir:
        ...
        print(f"ERROR: Must provide either existing project or output directory", flush=True)
        sys.exit(1)
    ...
    if args.quality.lower() not in ["low", "medium", "high"]:
        print(f"ERROR: Quality must be low, medium, or high", flush=True)
        sys.exit(1)
    ...
    if not os.path.exists(project_dir + "project.psx"):
        print(f"NOTE: Creating new project file", flush=True)
    ...
    if not chunk.cameras:
        ...
        if os.path.exists(args.input_dir):
            ...
            print("ERROR: Input directory provided doesn't exist; please check input", flush=True)
            sys.exit(1)
        ...
        if not photos:
            print(f"ERROR: Image directory provided does not contain any usable images; please check input", flush=True)
            sys.exit(1)

        print("\n###############################################", flush=True)
        print("Adding photos", flush=True)
        print("###############################################\n", flush=True)

        chunk.addPhotos(photos, progress=print_sfm_progress)  # No MT
        print(str(len(chunk.cameras)) + " images loaded", flush=True)
        doc.save()
    ...
    if chunk.depth_maps and not chunk.point_cloud:
        print("\n###############################################", flush=True)
        print("Building dense point cloud", flush=True)
        print("###############################################\n", flush=True)
    ...
    if chunk.model and not chunk.orthomosaic:
        print("\n###############################################", flush=True)
        print("Building orthomosaic", flush=True)
        print("###############################################\n", flush=True)
    ...
    if chunk.point_cloud and not os.path.exists(output_dense):
        print("\n###############################################", flush=True)
        print("Exporting dense point cloud", flush=True)
        print("###############################################\n", flush=True)
    ...
    if chunk.elevation and not os.path.exists(output_dem):
        print("\n###############################################", flush=True)
        print("Exporting DEM", flush=True)
        print("###############################################\n", flush=True)
    ...
    if chunk.orthomosaic and not os.path.exists(output_ortho):
        print("\n###############################################", flush=True)
        print("Exporting orthomosaic", flush=True)
        print("###############################################\n", flush=True)
