...
The CoralNet-Toolbox integrates a suite of SAM variants to suit different needs, including the original, high-accuracy SAM; the faster FastSAM; and the lightweight MobileSAM, which is optimized for speed and use on systems with less computational power. Furthermore, the toolbox incorporates other advanced AI-assisted annotation tools, further reducing the manual annotation burden.
...
| Visual Prompting / Zero-Shot Models | YOLOE (See Anything) | AI-assisted annotation; detecting objects based on visual examples or text prompts without prior training. |
...
2. **Modify or Extend Annotations:** The annotations are modified (e.g., by re-mapping labels), added on to (for rarer class categories), or used directly to train a patch-based image classification model.
3. **Export for Training:** The newly created classification dataset is exported in the specific YOLO format required for training a custom model locally.
4. **Model Deployment:** Using the pre-trained model, additional data (from existing or novel images) can be annotated to increase the total amount of training data, iteratively increasing model accuracy.
