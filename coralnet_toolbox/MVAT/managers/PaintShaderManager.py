...
class PaintShaderManager:
    ...
    def should_show_label_overlay(self, product) -> bool:
        ...
        NOT suppressed while the Feature Select similarity overlay is engaged:
        painted labels render ON TOP of the heatmap (the label overlay is added
        to the renderer after the similarity overlay, so it draws later in the
        translucent pass), still governed by the label transparency slider —
        mirroring the 2D stacking (label overlay z=-3 above feature z=-4).
        """
