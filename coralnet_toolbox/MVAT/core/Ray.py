...
class SphereActorManager:
    ...
    def __init__(self, radius: float = 0.1):
        ...
        self._create_sphere()
        # Set initial position to origin so sphere is visible there
    ...
    def _create_sphere(self):
        ...
        try:
            ...
            # Fallback: use the sphere directly (will be rendered as wireframe via actor)
    ...
    def set_position(self, position: np.ndarray):
        ...
        if self.current_position is None or not np.allclose(self.current_position, position):
            ...
            # Update sphere mesh points to new position
            # Get the offset from the original sphere center (at origin)
            original_points = self._original_sphere_mesh.points
            # Translate all points by the position vector
            self.sphere_mesh.points = original_points + position

            # Mark mesh as modified so the plotter updates
    ...
    def add_to_plotter(self, plotter, color: tuple = (144, 238, 144),
        ...
        if self.sphere_mesh is not None:
            ...
            # Check if mesh is already edges/wireframe
            ...
            if is_edges:
                ...
                # Render solid sphere in wireframe mode
                self.sphere_actor = plotter.add_mesh(
                    self.sphere_mesh,
                    color=norm_color,
                    style='wireframe',
