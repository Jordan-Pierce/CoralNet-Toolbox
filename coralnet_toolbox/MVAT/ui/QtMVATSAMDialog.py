...
3.  The user places SAM prompts (left-click = positive, right-click = negative,
    left-drag = bounding-box rectangle).
4.  Each prompt change triggers a live SAM prediction; the binary mask is
    painted as a semi-transparent colour overlay on the screenshot.
5.  Pressing Enter / clicking Accept emits ``maskAccepted(np.ndarray)`` with
    the full-resolution binary mask (same H×W as rgb_image / index_map).
6.  Pressing Escape / Cancel closes the dialog with no side-effects.
"""
...
from PyQt5.QtGui import (QColor, QImage, QPainter, QPen, QBrush,
                         QPixmap, QKeyEvent, QMouseEvent, QCursor)
...
def _numpy_to_qpixmap(rgb: np.ndarray) -> QPixmap:
    """Convert an H×W×3 uint8 numpy array to a QPixmap."""
    h, w, ch = rgb.shape
...
def _mask_to_qpixmap(binary_mask: np.ndarray, color: QColor) -> QPixmap:
    """
    Convert a boolean H×W mask to a semi-transparent colour QPixmap.
...
class MVATSAMDialog(QDialog):
    ...
    rgb_image   : np.ndarray, shape (H, W, 3), dtype uint8
        Screenshot of the 3D viewer at the moment Space was pressed.
    index_map   : np.ndarray, shape (H, W), dtype int32
        Pixel→element_id map built by VisibilityManager for the same view.
        Value -1 means background / no geometry.
    element_type : str
        ``'point'`` or ``'face'`` — determines how element IDs are interpreted.
    sam_dialog  : DeployPredictorDialog
        The application-wide SAM predictor (already has a model loaded).
    label       : Label
        The annotation label that will be applied on accept.
    parent      : QWidget, optional
    """

    # Emitted on Accept with the full-resolution binary mask (dtype bool, H×W)
    ...
    def __init__(self, rgb_image: np.ndarray, index_map: np.ndarray,
        ...
        self._rgb       = rgb_image          # H×W×3
        self._index_map = index_map          # H×W  int32
        ...
        # Current prediction
        self._binary_mask: np.ndarray | None = None
    ...
    def _build_ui(self):
        ...
        # Instructions
        info = QLabel(
            "<b>Left-click</b> positive &nbsp;|&nbsp; "
            "<b>Right-click</b> negative &nbsp;|&nbsp; "
            "<b>Left-drag</b> bounding box &nbsp;|&nbsp; "
            "<b>Backspace</b> clear prompts"
        )
        ...
        # Status line
        self._status = QLabel("Place prompts to segment the view.")
        ...
        # Size: fill 80 % of the screen, but not larger than the image
    ...
    # ------------------------------------------------------------------
    # Prompt handling  (called by _PromptView)
    # ------------------------------------------------------------------
    ...
    def update_rect(self, scene_pos: QPointF):
    ...
    def _predict(self):
        """Run SAM with current prompts and update the overlay."""
        ...
        labels_input  = [[1] * len(positive) + [0] * len(negative)] if all_pts else None
        ...
        conf_val = float(result.boxes.conf[top_idx]) if hasattr(result.boxes.conf[top_idx], '__float__') else 0.0
        self._status.setText(
            f"Mask covers {int(self._binary_mask.sum()):,} px  |  "
    ...
    def _count_elements(self) -> int:
        """Count unique element IDs inside the current mask."""
    ...
    def _update_overlay(self):
        ...
        from PyQt5.QtWidgets import QGraphicsPixmapItem as _GPI
        self._overlay_item = _GPI(pix)
...
class _PromptView(QGraphicsView):
    """GraphicsView that forwards prompt clicks/drags to MVATSAMDialog."""
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            ...
            # We'll decide click vs drag in release / move
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if event.buttons() & Qt.LeftButton and self._drag_origin is not None:
            sp   = self.mapToScene(event.pos())
            dist = (sp - self._drag_origin)
            if not self._drag_started and (abs(dist.x()) > 4 or abs(dist.y()) > 4):
                self._drag_started = True
                self._dialog.begin_rect(self._drag_origin)
            if self._drag_started:
                self._dialog.update_rect(sp)
