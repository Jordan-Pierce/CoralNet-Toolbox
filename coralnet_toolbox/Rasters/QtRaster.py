...
class Raster(QObject):
    ...
    def get_z_value(self, x: int, y: int) -> Optional[float]:
        """
        Get the raw z-channel value at a specific pixel coordinate.
        ...
        Returns:
            Optional[float]: Raw z-value, or None if out of bounds or no z-channel
        """
            ...
            if z_data_type == 'elevation' and self.transform_matrix is None:
                try:
                    with rasterio.open(z_channel_path) as dem_dataset:
                        self._extract_transform_matrix(source_dataset=dem_dataset)
                except Exception as e:
