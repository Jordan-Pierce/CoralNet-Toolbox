...
def smart_fill_z_channel(existing_z, predicted_z, existing_unit='meters', existing_type='depth'):
    ...
    Pipeline:
    1. Quantile Mapping (Non-Linear): Maps the distribution of the prediction to 
       match the ground truth (corrects non-linear depth compression).
    ...
    if existing_type == 'depth':
        valid_mask = ~np.isnan(z_existing) & (z_existing != 0)
        nan_mask = np.isnan(z_existing) | (z_existing == 0)
    else:
    ...
    if num_nans == 0:
        return z_existing, {'num_filled': 0, 'method': 'none'}
    ...
    # Compute percentiles (quantiles) for both distributions
    # Using 20 control points (5% steps) is usually enough to capture the curve without noise
    quantiles = np.linspace(0, 100, 21)
    gt_percentiles = np.percentile(valid_gt, quantiles)
    pred_percentiles = np.percentile(valid_pred, quantiles)
    
    # Use interpolation to map the entire predicted image to the ground truth distribution
    ...
    residual_map = np.zeros_like(z_existing, dtype=np.float32)
    residual_map[valid_mask] = z_existing[valid_mask] - z_non_linear[valid_mask]
    
    # 2. Normalize Residuals for cv2.inpaint (requires 8-bit)
    valid_residuals = residual_map[valid_mask]
    
    if valid_residuals.size > 0:
        r_min = np.min(valid_residuals)
        r_max = np.max(valid_residuals)
        r_range = r_max - r_min if (r_max - r_min) > 1e-6 else 1.0
        
        # Map residuals to 0-255
        residual_norm = ((residual_map - r_min) / r_range) * 255.0
        residual_uint8 = np.clip(residual_norm, 0, 255).astype(np.uint8)
        
        # 3. Inpaint the Residuals
        inpaint_mask = nan_mask.astype(np.uint8)
        # Radius 5 is usually good for blending seams
        inpainted_residual_uint8 = cv2.inpaint(residual_uint8, inpaint_mask, 5, cv2.INPAINT_TELEA)
        
        # 4. Denormalize back to float32
        correction_surface = (inpainted_residual_uint8.astype(np.float32) / 255.0) * r_range + r_min
        
        # 5. Apply Correction
        z_final = z_non_linear + correction_surface
    else:
    ...
    stats = {
        'num_valid': num_valid,
        'num_filled': num_nans,
        'method': 'Non-Linear Quantile Mapping + Residual Blending',
        'gt_range': (np.min(valid_gt), np.max(valid_gt)),
        'pred_range_original': (np.min(valid_pred), np.max(valid_pred))
    }
