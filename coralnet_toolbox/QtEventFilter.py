    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.KeyPress:
            if event.modifiers() & Qt.ControlModifier and not (event.modifiers() & Qt.ShiftModifier):
                ...
                # Handle WASD keys for selecting Label - these should always work
                if event.key() in [Qt.Key_W, Qt.Key_A, Qt.Key_S, Qt.Key_D]:
                    self.label_window.handle_wasd_key(event.key())
                    return True

                # --- Exit early if Explorer is active, effectively disabling all OTHER hotkeys below ---
                if hasattr(self.main_window, 'explorer_window'):
                    if self.main_window.explorer_window.isVisible():
                        return True
        ...
        # For all other key events (not Ctrl or with Shift+Ctrl), check if Explorer is visible 
        # to disable these hotkeys too
        if hasattr(self.main_window, 'explorer_window'):
            if self.main_window.explorer_window.isVisible():
                return False  # Let the explorer window handle these events
