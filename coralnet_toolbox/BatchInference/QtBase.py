...
class BatchInferenceDialog(QDialog):
    ...
    def setup_thresholds_layout(self):
        ...
        """
        # Default: show uncertainty threshold
    ...
    def update_thresholds_for_model(self, model_name):
        """
        Update the visibility and enabled state of thresholds based on model type.
    ...
    def configure_thresholds(self, show_max_detections, show_uncertainty,
                             show_iou, show_area):
        """
        Configure which thresholds are visible and enabled.
        
        :param show_max_detections: Whether to show max detections
        :param show_uncertainty: Whether to show uncertainty threshold
        :param show_iou: Whether to show IoU threshold
        :param show_area: Whether to show area threshold
        """
        # Hide/show max detections
        if hasattr(self.thresholds_widget, 'max_detections_spinbox'):
            parent = self.thresholds_widget.max_detections_spinbox.parent()
            self.thresholds_widget.max_detections_spinbox.setVisible(show_max_detections)
            # Also hide the associated label
            for child in parent.children():
                if hasattr(child, 'text') and 'Max Detections' in str(child.text()):
                    child.setVisible(show_max_detections)
        
        # Hide/show uncertainty threshold
        if hasattr(self.thresholds_widget, 'uncertainty_threshold_slider'):
            self.thresholds_widget.uncertainty_threshold_slider.setVisible(
                show_uncertainty
            )
            if hasattr(self.thresholds_widget, 'uncertainty_threshold_label'):
                self.thresholds_widget.uncertainty_threshold_label.setVisible(
                    show_uncertainty
                )
        
        # Hide/show IoU threshold
        if hasattr(self.thresholds_widget, 'iou_threshold_slider'):
            self.thresholds_widget.iou_threshold_slider.setVisible(show_iou)
            if hasattr(self.thresholds_widget, 'iou_threshold_label'):
                self.thresholds_widget.iou_threshold_label.setVisible(show_iou)
        
        # Hide/show area threshold
        if hasattr(self.thresholds_widget, 'area_threshold_min_slider'):
            self.thresholds_widget.area_threshold_min_slider.setVisible(show_area)
            self.thresholds_widget.area_threshold_max_slider.setVisible(show_area)
            if hasattr(self.thresholds_widget, 'area_threshold_label'):
                self.thresholds_widget.area_threshold_label.setVisible(show_area)
