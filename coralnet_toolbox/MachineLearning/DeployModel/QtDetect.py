...
class Detect(Base):
    ...
    def predict(self, image_paths=None):
        ...
        Args:
            image_paths: List of image paths to process.  If None, processes the
        ...
        try:
            for idx, image_path in enumerate(image_paths):
                ...
                if raster is None:
                    print(f"Detect.predict: no raster for {image_path}, skipping.")
                    continue

                # Determine work items: tiles when a work-area tool is active,
                # otherwise treat the whole image as one item.
                use_tiles = (
                ...
                for i in range(0, len(work_areas), self.BATCH_SIZE):
                    ...
                    # Build YOLO inputs for this mini-batch
                    inputs = []
                    for wa in wa_chunk:
                        if wa is not None:
                            wa.highlight()
                            ...
                            # Full-image: special-case virtual video-frame paths.
                            # If the current image_path is a virtual video frame
                            # (video.mp4::frame_N) fetch the raw BGR frame and
                            # pass the ndarray directly to the model to avoid
                            # giving Ultralytics a non-filesystem path.
                            if isinstance(image_path, str) and '::frame_' in image_path:
                                try:
                                    ...
                                    # Any failure falls back to letting YOLO open the path
                                    pass
                            # Fallback: let YOLO open the path (normal image or video file)
                            inputs.append(image_path)  # full-image: let YOLO do the I/O

                    batch_results = self._apply_model(inputs)

                    # Remap tile coordinates + collect
                ...
                # SAM pass: called once per image after all tiles are collected.
                # The same full-image array is set on every result so SAM's ViT
                # encoder fires only once (image-embedding cache hit).
                ...
                # Fast ghost-render so the user sees predictions immediately
                if image_path == self.annotation_window.current_image_path:
                    try:
                        ...
                        print(f"Detect.predict: fast render failed: {e}")

        except Exception as e:
            print(f"Detect.predict: fatal error: {e}")
            import traceback
            traceback.print_exc()
        finally:
            if cache:
